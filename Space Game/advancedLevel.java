...
public class advancedLevel extends JPanel implements KeyListener, ActionListener{
	Timer timer = new Timer(25, this);
	
	private String userName;
	private int score = 0;
	private spaceShip ship = new spaceShip();
	private BufferedImage shipImage, alienImage, bulletImage, blueAlienImage, purpleAlienImage;
	
	private ArrayList<bullet> bullets = new ArrayList<bullet>();
	private ArrayList<alien> aliens = new ArrayList<alien>();
	
	
	public advancedLevel(String userName) {
		...
		aliens.add(new blueAlien(0,300));
		aliens.add(new greenAlien(100,300));
		aliens.add(new greenAlien(450,100));
		aliens.add(new blueAlien(650,100));
		aliens.add(new greenAlien(550,100));
		aliens.add(new greenAlien(200,0));
		aliens.add(new blueAlien(300,0));
		aliens.add(new greenAlien(400,0));
		aliens.add(new greenAlien(100,-100));
		aliens.add(new purpleAlien(200,-100));
		aliens.add(new greenAlien(300,-100));
		aliens.add(new greenAlien(450,-100));
		aliens.add(new purpleAlien(650,-100));
		aliens.add(new greenAlien(550,-100));
		aliens.add(new greenAlien(100,-250));
		aliens.add(new blueAlien(200,-250));
		aliens.add(new greenAlien(550,-400));
		aliens.add(new greenAlien(350,-700));
		aliens.add(new blueAlien(350,-850));
		aliens.add(new greenAlien(550, -850));
		aliens.add(new blueAlien(350,-1000));
		aliens.add(new blueAlien(550,-1000));
		aliens.add(new greenAlien(650,-1000));
		aliens.add(new greenAlien(150,-1300));
		aliens.add(new blueAlien(550,-1200));
		aliens.add(new purpleAlien(650,-1500));
		aliens.add(new greenAlien(350,-1200));
		aliens.add(new purpleAlien(250,-1100));
		aliens.add(new purpleAlien(550,-1300));
		aliens.add(new greenAlien(150,-1500));
		...
		try {
			...
			blueAlienImage = ImageIO.read(new FileImageInputStream(new File("alien2_1.png")));
			purpleAlienImage = ImageIO.read(new FileImageInputStream(new File("alien3_1.png")));
	...
	public void paintComponent(Graphics g) {
		...
		for (int i = bullets.size() - 1; i >= 0; i--) {
	        ...
	        for (int j = aliens.size() - 1; j >= 0; j--) {
	            ...
	            if (bulletHitbox.getBounds().intersects(alienHitbox.getBounds())) {
            		...
            		if(aliens.get(j).getType().equals("green")) 
            			score += 5;
            		
            		else if(aliens.get(j).getType().equals("blue"))
            			score += 10;
            		
            		else if(aliens.get(j).getType().equals("purple"))
            			score += 15;
		...
		for(alien alien_ : aliens) {
			...
			else if(alien_.getType().equals("blue")) {
				g.drawImage(blueAlienImage, alien_.getX(), alien_.getY(), alienImage.getWidth() - 7, alienImage.getHeight() - 8, this);
			}
			
			else if(alien_.getType().equals("purple")) {
				g.drawImage(purpleAlienImage, alien_.getX(), alien_.getY(), alienImage.getWidth() - 7, alienImage.getHeight() - 8, this);
			}
		}
